...
각 추천안별 사업 가속 조건, 예상 마진 시나리오, 인허가 리스크 및 대한민국 주차장법 마진을 조화롭게 대조하여, 의사결정권자가 안심하고 최선의 결정을 내릴 수 있도록 실전 엔지니어링 브리핑 답변을 완성해 수록해 주시기 바랍니다.`;
      ...
      if (!response.ok) {
        if (response.status === 404) {
          throw new Error("GitHub Pages 등 정적 호스팅 환경에서는 백엔드 Node.js 서버(server.ts)가 동작하지 않으므로 실시간 Gemini AI 기능을 실행할 수 없습니다. AI 세대 조합 최적화 기능을 사용하려면 미리보기용 Cloud Run 링크를 사용하시거나, 로컬 환경에서 실행해 주세요.");
        }
